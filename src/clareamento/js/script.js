  ...
  if (leadForm) {
    leadForm.addEventListener('submit', async e => {
      ...
      try {
        ...
        if (response.ok) {
          console.group('📢 [META PIXEL] Evento LEAD - Formulário Limpeza');
          console.log('✅ Status: Sucesso no envio do webhook');
          if (typeof fbq === 'function') {
            try {
              fbq('trackCustom', 'Lead_Formulario_Airflow');
              console.log('🎯 Evento fbq("trackCustom", "Lead_Formulario_Airflow") disparado com sucesso pelo formulário.');
            } catch (err) {
              console.error('❌ Erro ao disparar fbq("trackCustom", "Lead_Formulario_Airflow"):', err);
            }
          } else {
            console.warn('⚠️ Atenção: A função fbq() não foi encontrada ou o bloqueador de anúncios está ativo.');
          }
          console.groupEnd();
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  wppLinks.forEach(link => {
    link.addEventListener('click', (e) => {
      ...
      console.group('📢 [META PIXEL] Evento LEAD - Clique WhatsApp Limpeza');
      console.log(`🔗 Link de destino: ${href}`);
      // Verifica se a função do Pixel existe para disparar o evento
      if (typeof fbq === 'function') {
        try {
          fbq('trackCustom', 'Lead_WhatsApp_Airflow');
          console.log('🎯 Evento fbq("trackCustom", "Lead_WhatsApp_Airflow") disparado com sucesso pelo botão WhatsApp.');
        } catch (err) {
          console.error('❌ Erro ao disparar fbq("trackCustom", "Lead_WhatsApp_Airflow"):', err);
        }
      } else {
        console.warn('⚠️ Atenção: A função fbq() não foi encontrada ou o bloqueador de anúncios está ativo.');
      }
      ...
      setTimeout(() => {
        const redirectParams = new URLSearchParams({
          ...
          source: 'Landing Page Clareamento',
